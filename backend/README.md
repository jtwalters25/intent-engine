...
│   ├── prophecy_agent.py    # Time-aware intent scheduling and prediction
...
### Prophecy Agent (`intent_engine/prophecy_agent.py`)

Time-aware intent scheduling and prediction. Predicts when intent should shift based on temporal patterns and scheduled rules.

- **Time context inference** — classifies any datetime into one of six contexts: EARLY_MORNING, MORNING, AFTERNOON, EVENING, BEDTIME, LATE_NIGHT
- **Schedule management** — define intent templates with time ranges, day-of-week filters, and midnight-crossing support
- **Shift prediction** — predicts the next intent shift from both schedules and natural context transitions
- **Proactive suggestions** — warns before an upcoming shift (configurable window, default 15 min)
- **Override learning** — tracks when users override scheduled intents for future pattern analysis
